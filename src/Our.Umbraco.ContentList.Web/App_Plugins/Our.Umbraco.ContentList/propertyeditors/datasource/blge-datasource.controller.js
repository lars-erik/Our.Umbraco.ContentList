    ...
    [
      ...
      function (scope, dataSourceService, dataSourceState) {
        function dataSourcesLoaded(sources) {
          ...
          dataSourceState.all = scope.datasources;
          scope.values = scope.datasources.map(keyNameToIdValue);
          scope.datasourceProperty.config.items = scope.values;
        ...
        if ((dataSourceState.all || []).length === 0) {
          dataSourceService.getDataSources().then(dataSourcesLoaded);
        } else {
          scope.datasources = dataSourceState.all;
          scope.values = scope.datasources.map(keyNameToIdValue);
        }
